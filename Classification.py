...
"""
This script is used to train and test the model. It predicts the number of spikes in EEG/MEG trials. 

Usage: type "from Classification import <class>" to use one of its classes.
...
class ClassificationTransformer():
    ...
    def train(self, config, cost_sensitive, lambd, weighting_method, beta, weight_decay, amsgrad,\
              scheduler, warmup, patience, gpu_id, save, model_path, config_path, checkpoint):

        """
        Train a classification model on the training and validation set.
        ...
        Args:
            ...
            weighting_method (str): Method to weight the loss to counter imbalance effect. If None, no weighting is done,
            beta (float): Beta value to compute weights if weighted_method = 'ENS' (see utils.py),
        ...
        self.train_dataloader = get_dataloader(self.train_data, self.train_labels, batch_size, num_workers)
        self.validation_dataloader = get_dataloader(self.val_data, self.val_labels, batch_size, num_workers) 
        ...
        train_info = dict((e,{"Loss": 0, "Accuracy": 0, "F1_score": 0, "weighted_F1_score": 0}) for e in range(n_epochs))
        val_info = dict((e,{"Loss": 0, "Accuracy": 0, "F1_score": 0, "weighted_F1_score": 0}) for e in range(n_epochs))
        ...
        for e in range(n_epochs):
            ...
            train_loss = 0
            correct, total = 0,0
            F1_train, weighted_F1_train = 0,0
            train_steps = 0
            ...
            for i, (data, labels) in enumerate(self.train_dataloader):
                ...
                if mix_up:
                    ...
                    loss = (lambdas.squeeze()).to(device)*self.train_criterion_cls(mix_outputs, labels) +\
                    (1-lambdas.squeeze()).to(device)*self.train_criterion_cls(mix_outputs, rolled_labels)
                    loss = loss.sum()
                    #if (i > 4)&(i<8):
                        #print('i: ',i,'loss: ',loss)
                    # Backward
                    ...
                    total += labels.size(0)
                    F1_train += f1_score(y_pred.cpu().detach(), labels.cpu().detach(), average = 'macro', num_classes = self.n_classes)
                    weighted_F1_train += f1_score(y_pred.cpu().detach(), labels.cpu().detach(), average = 'weighted', num_classes = self.n_classes)
                    train_steps += 1
            ...
            train_info[e]["weighted_F1_score"] = weighted_F1_train / train_steps

            # Evaluate the model
            val_loss = 0
            val_correct, val_total = 0,0
            F1_val, weighted_F1_val = 0,0
            val_steps = 0
            ...
            for j, (val_data, val_labels) in enumerate(self.validation_dataloader):
                ...
                F1_val += f1_score(val_y_pred.cpu().detach(), val_labels.cpu().detach(), average = 'macro', num_classes = self.n_classes)
                weighted_F1_val += f1_score(val_y_pred.cpu().detach(), val_labels.cpu().detach(), average = 'weighted', num_classes = self.n_classes)
                val_steps += 1
            ...
            val_info[e]["weighted_F1_score"] = weighted_F1_val / val_steps
            
            # Update learning rate if validation loss does not improve
            if scheduler:
                self.lr_scheduler.step(val_loss / val_steps)
        ...
        print('Mean weighted F1 score on validation set:', "%.4f" % round(np.float(np.mean([val_info[e]["weighted_F1_score"] for e in range(final_epoch)])), 4))
        ...
        print('Final weighted F1 score on validation set:', "%.4f" % round(np.float(val_info[final_epoch-1]["weighted_F1_score"]), 4))
