...
from common_spatial_pattern import common_spatial_pattern, whitening
    ...
    def cross_validation(self, config, save, path_output, gpu_id):
        ...
        if intra_subject:
            ...
            for fold, (train_ids, test_ids) in enumerate(rkf.split(index)):
                ...
                target_mean = np.mean(train_data, axis=(1, 2))
                target_std = np.std(train_data, axis=(1, 2))
                train_data = ((train_data-target_mean[:, None, None])
                              / target_std[:, None, None])

                # Apply spatial filter
                n_rows = all_data.shape[1]
                valid_selection = (n_rows > 2*selected_rows)
                selected_rows = int(selected_rows*valid_selection
                                    + (n_rows/2)*(1-valid_selection))
                config['selected_rows'] = selected_rows
                self.spatial_filter = common_spatial_pattern(train_data,
                                                             train_labels,
                                                             selected_rows)
                train_data = np.einsum('c d, n d t -> n c t',
                                       self.spatial_filter, train_data)
                ...
                # Z-score standardization
                target_mean = np.mean(test_data, axis=(1, 2))
                target_std = np.std(test_data, axis=(1, 2))
                test_data = ((test_data-target_mean[:, None, None])
                             / target_std[:, None, None])
            ...
            # Print fold results
            print(f'K-FOLD CROSS VALIDATION RESULTS FOR'
                  '{n_splits*n_repeats} FOLDS')
            print('--------------------------------')
            ...
            if save:
                ...
                save_path = path_output + 'model_' + subject_id
                torch.save(models[best_fold], save_path)
        ...
        # Cross-subject learning
            ...
            for fold, (train_ids, test_ids) in enumerate(rkf.split(index)):
                ...
                # Check that not too much rows are selected
                selected_rows = CSP_projection_config['selected_rows']
                n_rows = min([all_data[id].shape[1] for id in self.subject_ids])
                valid_selection = (n_rows > 2*selected_rows)
                selected_rows = int(selected_rows*valid_selection + (n_rows/2)*(1-valid_selection))

                # DONT CONCATENATE BUT FEED AS A LIST TO THE DATALOADER !!!!!!!!!!!
                # Apply CSP projection
                train_data = [np.einsum('cd,ndt -> nct', common_spatial_pattern(all_data[id], all_labels[id], selected_rows), all_data[i])
                             for id in train_subject_ids]

                # Z-score standardization for each subject
                train_data = [(self.all_data[id]-np.mean(self.all_data[id]) / np.std(self.all_data[id])) for id in train_subject_ids]
                ...
                # Apply CSP projection
                test_data = [np.einsum('cd,ndt -> nct', common_spatial_pattern(all_data[id], all_labels[id], selected_rows), all_data[i])
                             for id in test_subject_ids]
                whitening_matrix = whitening(train_data, selected_rows)
                train_data = np.einsum('c d, n d t -> n c t', whitening_matrix, train_data)
                test_data = np.einsum('c d, n d t -> n c t', whitening_matrix, test_data)
                # Normalize for each subject
                test_data = [(self.all_data[id]-np.mean(self.all_data[id]) / np.std(self.all_data[id])) for id in test_subject_ids]
